...
//        videoRv.setLayoutManager(new LinearLayoutManager(this));
    ...
    private void download1() {
        ...
        ref  = storageReference.child("materipdf/IPA SEM 2.pdf");
        ref.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
            @Override
            public void onSuccess(Uri uri) {
                String url = uri.toString();
                donwloadFile(SainsActivity.this,"Materi_IPA_KelasVII_UBS_SEM2",".pdf", DIRECTORY_DOWNLOADS, url);
            }
    ...
    public void download() {
        storageReference = firebaseStorage.getInstance().getReference();
        ref  = storageReference.child("materipdf/IPA SEM 1.pdf");
        ref.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
            @Override
            public void onSuccess(Uri uri) {
                String url = uri.toString();
                donwloadFile(SainsActivity.this,"Materi_IPA_KelasVII_UBS_SEM1",".pdf", DIRECTORY_DOWNLOADS, url);
            }
        }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
